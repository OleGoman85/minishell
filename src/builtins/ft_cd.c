...
static int update_env_vars(char **current_pwd, char *oldpwd_val, t_shell *shell)
{
	...
	if (!(*current_pwd = getcwd(buffer, PATH_MAX)))
	{
		if (errno == ENOENT)
		{
			error_msg("cd: error retrieving current directory: ", "getcwd: cannot access parent directories: ", strerror(errno), shell);
...
static int process_cd_args(t_cmd *cmd, char **target_dir, t_shell *shell)
{
	...
	if (cmd->cmd_args[1] == NULL)
	{
		...
		if (env_var == NULL)
			return (error_msg("cd: ", NULL, "HOME environment variable is not set", shell));
	...
	else if (ft_strcmp(cmd->cmd_args[1], "-") == 0)
	{
		...
		if (env_var == NULL)
			return (error_msg("cd: ", NULL, "OLDPWD environment variable is not set", shell));
		*target_dir = strdup_tracked(get_ev_value(env_var), COMMAND_TRACK, shell);
	}
	else
...
int ft_cd(t_cmd *cmd, t_shell *shell)
{
	...
	if (target_dir != NULL && chdir(target_dir) == -1)
		return (error_msg("cd: ", target_dir, strjoin_tracked(": ", strerror(errno), COMMAND_TRACK, shell), shell));
	if (update_env_vars(&current_pwd, get_ev_value(get_ev("PWD", shell->ev_list)), shell) != EXIT_SUCCESS)
		return (EXIT_FAILURE);
	if (cmd->cmd_args[1] && ft_strcmp(cmd->cmd_args[1], "-") == 0)
		write_and_track(strjoin_tracked(current_pwd, "\n", COMMAND_TRACK, shell), STDOUT_FILENO, shell);
	return (EXIT_SUCCESS);
}
